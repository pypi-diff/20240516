# Comparing `tmp/routput-0.9997.tar.gz` & `tmp/routput-0.9998.tar.gz`

## filetype from file(1)

```diff
@@ -1 +1 @@
-gzip compressed data, was "routput-0.9997.tar", last modified: Thu May 16 14:41:38 2024, max compression
+gzip compressed data, was "routput-0.9998.tar", last modified: Thu May 16 14:43:59 2024, max compression
```

## Comparing `routput-0.9997.tar` & `routput-0.9998.tar`

### file list

```diff
@@ -1,18 +1,18 @@
-drwxrwxrwx   0        0        0        0 2024-05-16 14:41:38.183943 routput-0.9997/
--rw-rw-rw-   0        0        0    25652 2024-04-18 08:26:04.000000 routput-0.9997/LICENCE
--rw-rw-rw-   0        0        0     2597 2024-05-16 14:41:38.159559 routput-0.9997/PKG-INFO
--rw-rw-rw-   0        0        0     2083 2024-04-18 09:01:17.000000 routput-0.9997/README.md
-drwxrwxrwx   0        0        0        0 2024-05-16 14:41:38.183943 routput-0.9997/routput/
--rw-rw-rw-   0        0        0     2687 2024-05-16 14:41:38.000000 routput-0.9997/routput/_README.py
--rw-rw-rw-   0        0        0       22 2024-05-16 10:24:13.000000 routput-0.9997/routput/__init__.py
--rw-rw-rw-   0        0        0       31 2024-04-18 09:05:31.000000 routput-0.9997/routput/__main__.py
--rw-rw-rw-   0        0        0     3509 2024-05-16 14:41:38.000000 routput-0.9997/routput/_pickled.py
--rw-rw-rw-   0        0        0     8179 2024-04-18 09:10:57.000000 routput-0.9997/routput/routput.py
-drwxrwxrwx   0        0        0        0 2024-05-16 14:41:38.158537 routput-0.9997/routput.egg-info/
--rw-rw-rw-   0        0        0     2597 2024-05-16 14:41:38.000000 routput-0.9997/routput.egg-info/PKG-INFO
--rw-rw-rw-   0        0        0      275 2024-05-16 14:41:38.000000 routput-0.9997/routput.egg-info/SOURCES.txt
--rw-rw-rw-   0        0        0        1 2024-05-16 14:41:38.000000 routput-0.9997/routput.egg-info/dependency_links.txt
--rw-rw-rw-   0        0        0        8 2024-05-16 14:41:38.000000 routput-0.9997/routput.egg-info/top_level.txt
--rw-rw-rw-   0        0        0       42 2024-05-16 14:41:38.160576 routput-0.9997/setup.cfg
--rw-rw-rw-   0        0        0      498 2024-05-16 14:41:03.000000 routput-0.9997/setup.py
--rw-rw-rw-   0        0        0    22763 2024-05-16 14:41:38.000000 routput-0.9997/templated_setup.py
+drwxrwxrwx   0        0        0        0 2024-05-16 14:43:59.378843 routput-0.9998/
+-rw-rw-rw-   0        0        0    25652 2024-04-18 08:26:04.000000 routput-0.9998/LICENCE
+-rw-rw-rw-   0        0        0     2597 2024-05-16 14:43:59.353768 routput-0.9998/PKG-INFO
+-rw-rw-rw-   0        0        0     2083 2024-04-18 09:01:17.000000 routput-0.9998/README.md
+drwxrwxrwx   0        0        0        0 2024-05-16 14:43:59.378843 routput-0.9998/routput/
+-rw-rw-rw-   0        0        0     2687 2024-05-16 14:43:59.000000 routput-0.9998/routput/_README.py
+-rw-rw-rw-   0        0        0       22 2024-05-16 10:24:13.000000 routput-0.9998/routput/__init__.py
+-rw-rw-rw-   0        0        0       31 2024-04-18 09:05:31.000000 routput-0.9998/routput/__main__.py
+-rw-rw-rw-   0        0        0     3509 2024-05-16 14:43:59.000000 routput-0.9998/routput/_pickled.py
+-rw-rw-rw-   0        0        0     8179 2024-04-18 09:10:57.000000 routput-0.9998/routput/routput.py
+drwxrwxrwx   0        0        0        0 2024-05-16 14:43:59.353265 routput-0.9998/routput.egg-info/
+-rw-rw-rw-   0        0        0     2597 2024-05-16 14:43:59.000000 routput-0.9998/routput.egg-info/PKG-INFO
+-rw-rw-rw-   0        0        0      275 2024-05-16 14:43:59.000000 routput-0.9998/routput.egg-info/SOURCES.txt
+-rw-rw-rw-   0        0        0        1 2024-05-16 14:43:59.000000 routput-0.9998/routput.egg-info/dependency_links.txt
+-rw-rw-rw-   0        0        0        8 2024-05-16 14:43:59.000000 routput-0.9998/routput.egg-info/top_level.txt
+-rw-rw-rw-   0        0        0       42 2024-05-16 14:43:59.354941 routput-0.9998/setup.cfg
+-rw-rw-rw-   0        0        0      498 2024-05-16 14:43:25.000000 routput-0.9998/setup.py
+-rw-rw-rw-   0        0        0    22709 2024-05-16 14:43:59.000000 routput-0.9998/templated_setup.py
```

### Comparing `routput-0.9997/LICENCE` & `routput-0.9998/LICENCE`

 * *Files identical despite different names*

### Comparing `routput-0.9997/PKG-INFO` & `routput-0.9998/PKG-INFO`

 * *Files 2% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: routput
-Version: 0.9997
+Version: 0.9998
 Summary: recursively searches for files based on their extensions starting from a specified directory. It can print the directory structure, include or exclude specific files, use syntax highlighting for output, and anonymize file paths for privacy.
 Author: matrikater (Joel Watson)
 Description-Content-Type: text/markdown; charset=UTF-8; variant=GFM
 License-File: LICENCE
 
 # Recursive Output Utility
 
@@ -86,9 +86,9 @@
 python routput.py -e [py] -b
 ```
 
 ## License
 
 Open-source software licensed under GPL-2 license.
 
-## V0.9997 released on 16th/5/2024
+## V0.9998 released on 16th/5/2024
 hopefully `templated_setup` works now...
```

### Comparing `routput-0.9997/README.md` & `routput-0.9998/README.md`

 * *Files identical despite different names*

### Comparing `routput-0.9997/routput/_README.py` & `routput-0.9998/routput/_README.py`

 * *Files identical despite different names*

### Comparing `routput-0.9997/routput/_pickled.py` & `routput-0.9998/routput/_pickled.py`

 * *Files 1% similar despite different names*

```diff
@@ -1 +1 @@
-__INST__ = """"""
+__INST__ = """"""
```

### Comparing `routput-0.9997/routput/routput.py` & `routput-0.9998/routput/routput.py`

 * *Files identical despite different names*

### Comparing `routput-0.9997/routput.egg-info/PKG-INFO` & `routput-0.9998/routput.egg-info/PKG-INFO`

 * *Files 2% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: routput
-Version: 0.9997
+Version: 0.9998
 Summary: recursively searches for files based on their extensions starting from a specified directory. It can print the directory structure, include or exclude specific files, use syntax highlighting for output, and anonymize file paths for privacy.
 Author: matrikater (Joel Watson)
 Description-Content-Type: text/markdown; charset=UTF-8; variant=GFM
 License-File: LICENCE
 
 # Recursive Output Utility
 
@@ -86,9 +86,9 @@
 python routput.py -e [py] -b
 ```
 
 ## License
 
 Open-source software licensed under GPL-2 license.
 
-## V0.9997 released on 16th/5/2024
+## V0.9998 released on 16th/5/2024
 hopefully `templated_setup` works now...
```

### Comparing `routput-0.9997/templated_setup.py` & `routput-0.9998/templated_setup.py`

 * *Files 2% similar despite different names*

```diff
@@ -697,19 +697,17 @@
 			raise Exception("Could not find the `_pickled` module in the current directory.")
 		c = pickle_x_loads(base64_x_b64decode(_pickled.__INST__.encode()))
 		version = c["version"]
 		author = c["author"]
 		description = c["description"]
 		long_description = c["long_description"]
 		kwargs_for_setup_tools = c["kwargs_for_setup_tools"]
-		if not kwargs_for_setup_tools.get("py_modules"):
-			kwargs_for_setup_tools["py_modules"] = []
 
 		os.remove(f"{name}{os.path.sep}_pickled.py")
-		os.remove(f"templated_setup.py")
+		shutil.move(f"templated_setup.py", os.path.join(name, "templated_setup.py"))
 		os.remove(f"{name}{os.path.sep}_README.py")
 
 		setup(
 			name=name,
 			version=version.version_number,
 			author=author,
 			description=description,
```

